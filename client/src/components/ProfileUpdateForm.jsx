...
function ProfileUpdateForm({ handleCapture, handleInputChange, handleFileChange, formData, setFormData }) {
  ...
  useEffect(() => {
    if (selectedDistrictOption) {
      const districtData = locationData?.data.find(
        (district) => district.district === selectedDistrictOption.value
      );
      ...
      const currentPanchayatIsValid = filteredPanchayatsOptions.some(
        (p) => p.value === formData.panchayat
      );
    ...
    <Form className=" py-sm-1  px-sm-5  bg-white">
      ...
      <Row className="mb-1 ">
        <Form.Group as={Col} xs={12} md={6} controlId="formAddImages" className="p-2 ">
          ...
          <Form.Control type="file" accept="image/*" onChange={handleFileChange} required className="border-success" />
      ...
      <Row className="mb-1">
  <Form.Group as={Col} xs={12} md={12} controlId="formGender" className="p-2"> {/* Changed controlId */}
    <Form.Label className="fw-bolder text-dark mb-1">
...
<Row className="mb-1">
  <Form.Group as={Col} xs={12} md={12} controlId="formCategory" className="p-2"> 
    ...
    <Select
      ...
      value={
        formData.category 
          ? category.find((option) => option.value === formData.category)
          : null
      }
      onChange={(selectedOption) => {
